/**
 * @title Deploy IdRegistry to Base Sepolia
 * @notice Deployment script for IdRegistry contract
 */
...
async function main() {
  ...
  console.log('========================================');
  console.log('Deploying IdRegistry to Base Sepolia');
  console.log('========================================');
  ...
  const artifactsPath = path.join(__dirname, '../artifacts/contracts/IdRegistry.sol/IdRegistry.json');
  ...
  const newEnvVar = `ID_REGISTRY_ADDRESS=${address}`;
  if (envContent.includes('ID_REGISTRY_ADDRESS')) {
    envContent = envContent.replace(/ID_REGISTRY_ADDRESS=.*/, newEnvVar);
  } else {
    envContent += `\n${newEnvVar}`;
  }
